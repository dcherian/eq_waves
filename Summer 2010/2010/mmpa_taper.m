...
a2=1; % alpha^2 -> Trade off paramenter
F13(1:n_modes,1) = 0;
F13(1:n_modes-1,2:n_modes) = (-1)*eye(n_modes-1);
F11 = eye(n_modes) + F13;
S = a2*F11'*F11;
Fpinvb = inv(Fu'*Fu + inv(S))*Fu';
%Fpinv = inv(Fu(:,2:end)'*Fu(:,2:end) + a^2*eye(n_modes-1))*Fu(:,2:end)';

% Fit barotropic mode to full U/V
Ufit = (detrend(Ufit','constant'))';
Aub = Fpinvb*Ufit;
Avb = Fpinvb*Vfit;
figure; % Energy plot
hold on
plot([0:n_modes-1],mean(Aub'.^2),'r');
plot([0:n_modes-1],mean(Avb'.^2),'g');
title('red = Ufit, green = Vfit, 0 = barotropic mode');
figure
plot(Avb');
title('Avb');
...
f_width = 3; 
delta_t = 2;%10/60/24; % in units of days

% remember first column is barotropic mode
%coher(Au(:,1),Au(:,2),delta_t,f_width,'mode1','mode2',1);

[u1,u2, coheramp_out,coherpha_out,freq] = coher(Aub(2,:),Aub(3,:),delta_t,f_width,'U mode1',' U mode2',1);
[v1,v2, coheramp_out,coherpha_out,freq] = coher(Avb(2,:),Avb(3,:),delta_t,f_width,'V mode1',' V mode2',1);
